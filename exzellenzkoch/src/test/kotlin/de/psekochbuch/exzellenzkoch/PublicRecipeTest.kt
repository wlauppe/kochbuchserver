...
class PublicRecipeTest {
    ...
    @Autowired
    val publicRecipeService : PublicRecipeService? = null
    ...
    fun addRecipe()
    {
        ...
        val recipe = publicRecipeService?.addRecipe(newRec)
    ...
    {
        if(id != 0) {
            val recipe = publicRecipeService?.getRecipe(id)
            Assertions.assertThat(newRec.equals(recipe))
        }
    }
    ...
    fun updateRecipe()
    {
        if(newRec.id != 0)
        {
            ...
            publicRecipeService?.updateRecipe(newRec,newRec.id)
        }
    }
